...
/*   Updated: 2023/12/09 16:35:25 by abazerou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void is_path_valid(t_var *v)
{
    ...
    while (i < v->i)
    {
        ...
        v->n = 0;
        v->x = 0;
        while (v->new_map[i][v->y] && (v->new_map[i][v->y] == ' ' || v->new_map[i][v->y] == '\t'))
            v->y++;
        if (v->new_map[i][v->y] != '\0')
        {
            v->y = 0;
            get_path(v, i, v->y);
            if (v->flag == 1)
...
int check_id(t_var *v)
{
    ...
    while (v->x < v->i)
    {
        v->y = 0;
        v->n = 0;
        while (v->new_map[v->x][v->y] && (v->new_map[v->x][v->y] == ' ' || v->new_map[v->x][v->y] == '\t'))
            v->y++;
